...
def fetch_polymarket_markets(asset: str, limit: int = 200) -> list:
    ...
    with st.spinner(f"Searching for {asset} 5-min markets..."):
        while len(markets) < limit:
            ...
            for m in page:
                ...
                # Filter to 5-min up/down markets for the right asset
                if (asset.lower() in question and
                    ("5" in question or "5m" in question or "5-min" in question) and
    ...
    if len(markets) < 10:
        try:
            url = f"{GAMMA_API}/markets?closed=true&limit=200&slug={slug_prefix}"
            resp = requests.get(url, timeout=15)
            resp.raise_for_status()
            markets = resp.json() or []
        except Exception as e:
            st.warning(f"⚠️ Slug search failed: {e}")
